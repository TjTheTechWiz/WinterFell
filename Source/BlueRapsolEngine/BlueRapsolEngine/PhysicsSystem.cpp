...
void PhysicsSystem::UpdatePhysics(const std::vector<std::unique_ptr<GameObject>>& getObjRef, int player1Index, int player2Index, float deltaTime) {
	...
	//Establish Hazard B bounds
	widthHolder = abs(getObjRef[4].get()->GetPhysicsComponent()->GetBounds().min.x - getObjRef[4].get()->GetPhysicsComponent()->GetBounds().max.x) / 2;
	heightHolder = abs(getObjRef[4].get()->GetPhysicsComponent()->GetBounds().min.y - getObjRef[4].get()->GetPhysicsComponent()->GetBounds().max.y) / 2;

	hazardBounds.min.x = getObjRef[4].get()->GetTransformComponent()->GetPosition().x;
	hazardBounds.max.x = getObjRef[4].get()->GetTransformComponent()->GetPosition().x + widthHolder;
	hazardBounds.min.y = getObjRef[4].get()->GetTransformComponent()->GetPosition().y + heightHolder; //Note: Remember, In this case Y positive is downwards
	hazardBounds.max.y = getObjRef[4].get()->GetTransformComponent()->GetPosition().y; //and Y negative is upwards

	hazardVelocityHolder.x = getObjRef[4].get()->GetPhysicsComponent()->GetVelocity().x;
	hazardVelocityHolder.y = getObjRef[4].get()->GetPhysicsComponent()->GetVelocity().y;
	...
	if (TestAABBOverlap(&hazardBounds, &wallBounds)) {
		...
		//Deflect upon collision
		if ((hazardBounds.max.x > wallBounds.min.x) && (hazardBounds.max.x < wallBounds.min.x + hazardPadding)) { // Hazard B collides with Wall from left
			OutputDebugString(L"[Notice] Hazard C collides with object from left.\n");
			if (getObjRef[i].get()->tag == TagType::Wall && hazardVelocityHolder.x > 0) {
				getObjRef[4].get()->GetPhysicsComponent()->SetVelocity(hazardVelocityHolder.x * -1, hazardVelocityHolder.y);
			}
		}
		else if ((hazardBounds.min.x < wallBounds.max.x) && (hazardBounds.min.x > wallBounds.max.x - hazardPadding)) { // Hazard B collides with Wall from right
			OutputDebugString(L"[Notice] Hazard C collides with object from right.\n");
			if (getObjRef[i].get()->tag == TagType::Wall && hazardVelocityHolder.x < 0) {
				getObjRef[4].get()->GetPhysicsComponent()->SetVelocity(hazardVelocityHolder.x * -1, hazardVelocityHolder.y);
			}
		}
		else if ((hazardBounds.max.y < wallBounds.min.y) && (hazardBounds.max.y > wallBounds.min.y - hazardPadding)) { // Hazard B collides with Wall from below
			OutputDebugString(L"[Notice] Hazard C collides with object from below.\n");
			if (getObjRef[i].get()->tag == TagType::Wall && hazardVelocityHolder.y < 0) {
				getObjRef[4].get()->GetPhysicsComponent()->SetVelocity(hazardVelocityHolder.x, hazardVelocityHolder.y * -1);
			}
		}
		else if ((hazardBounds.min.y > wallBounds.max.y) && (hazardBounds.min.y < wallBounds.max.y + hazardPadding)) { // Hazard B collides with Wall from above
			OutputDebugString(L"[Notice] Hazard C collides with object from above.\n");
			if (getObjRef[i].get()->tag == TagType::Wall && hazardVelocityHolder.y > 0) {
				getObjRef[4].get()->GetPhysicsComponent()->SetVelocity(hazardVelocityHolder.x, hazardVelocityHolder.y * -1);
			}
		}
