...
void BlueRapsolApp::GameStart() {
	...
	Instantiate("Hazard B", "BeybladeTex", 410, 160, 20, 20, TagType::Hazard);
	allObjects[getObjIndex["Hazard C"]].get()->tag = TagType::Hazard;

	Instantiate("Hazard B", "BeybladeTex", 190, 160, 10, 10, TagType::Hazard);
	allObjects[getObjIndex["Hazard D"]].get()->tag = TagType::Hazard;
...
void BlueRapsolApp::Rematch() {
	...
	allObjects[getObjIndex["Hazard C"]].get()->GetPhysicsComponent()->SetVelocity(0.4 * hazardSpeed, 0.8 * hazardSpeed);
	allObjects[getObjIndex["Hazard D"]].get()->GetPhysicsComponent()->SetVelocity(0.8 * hazardSpeed, 0.4 * hazardSpeed);
	//reenable players
	allObjects[getObjIndex["Player1"]].get()->isEnabled = true;
	allObjects[getObjIndex["Player2"]].get()->isEnabled = true;
